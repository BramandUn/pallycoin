...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop Pallycoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Pallycoin server stopping";
}
...
{ //  category              name                      actor (function)         okSafeMode threadSafe reqWallet
  //  --------------------- ------------------------  -----------------------  ---------- ---------- ---------
    ...
    /* Pallycoin features */
    { "pallycoin",               "masternode",             &masternode,             true,      true,       false },
    { "pallycoin",               "masternodelist",         &masternodelist,         true,      true,       false },
    { "pallycoin",               "mnbudget",               &mnbudget,               true,      true,       false },
    { "pallycoin",               "mnbudgetvoteraw",        &mnbudgetvoteraw,        true,      true,       false },
    { "pallycoin",               "mnfinalbudget",          &mnfinalbudget,          true,      true,       false },
    { "pallycoin",               "mnsync",                 &mnsync,                 true,      true,       false },
    { "pallycoin",               "spork",                  &spork,                  true,      true,       false },
#ifdef ENABLE_WALLET
    { "pallycoin",               "darksend",               &darksend,               false,     false,      true  }, /* not threadSafe because of SendMoney */
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("To use pallycoind, or the -server option to pallycoin-qt, you must set an rpcpassword in the configuration file:\n"
              "%s\n"
...
std::string HelpExampleCli(string methodname, string args){
    return "> pallycoin-cli " + methodname + " " + args + "\n";
}
